...
def _run_footer(run_url: str) -> str:
    return (
        "---\n\n<sub>Generated by the [site submission workflow]"
        f"({run_url}). Raw artifacts (screenshot, detection details) are downloadable "
        "from that run for 30 days.</sub>"
    )
...
def build_pr_body(
    ...
    if not p.developer_exists and logo_expected:
        lines += [
            ...
            "",
            f"![Logo of {p.developer_name}](https://raw.githubusercontent.com/{repo_full_name}/{branch}/{paths['logo']})",
            "",
        ]
    lines += [
        "### Site entry",
        ...
        "### How to review",
        "",
        f"Run `just serve`, then visit `/developers/{p.developer_slug}/{p.site_slug}` locally.",
        "",
        f"Closes #{p.issue_number}",
        _run_footer(run_url),
    ]
    return "\n".join(lines)
